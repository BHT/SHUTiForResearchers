...
date: "Compiled for GitHub August 7, 2023"
output:
...
If you choose to download this file onto your personal machine, please take care to periodically refer back to the project's github page at <https://github.com/BHT/SHUTiForResearchers> for potential updates to the below syntax. <<You are currently viewing _v3 of the syntax>>. A full description of updates between each version will be saved as a log on the project's github page. In addition, updates from the last version to the current version will be annotated throughout this syntax. 

V3 was built using R version 4.2.3 (2023-03-15 ucrt) -- "Shortstop Beagle"
...
Our team developed a companion manuscript to further describe these data management processes, which is available at: https://doi.org/10.1093/sleep/zsad199. The authors' copy of the accepted (uncopyedited) manuscript is available for download on the SHUTi for Researchers GitHub page. 


## Citations

For any presentations or published works that leverages data management processes and/or files shared here, please cite BOTH our github project page AND the companion manuscript:

Shaffer, K.M., Daniel, K.E., & Ritterband, L.M.  (2023). Sleep Diary Data - Formatting and Outcomes Computation for SHUTi for Researchers (Version 3.0) [Source code]. https://github.com/BHT/SHUTiForResearchers

Shaffer, K. M., Daniel, K. E., Frederick, C., Buysse, D. J., Morin, C. M., & Ritterband, L. M. (2023). Online Sleep Diaries: Considerations for System Development and Recommendations for Data Management. Sleep, zsad199.
...
##EDITING REQUIRED: There are two syntax options for you to use below. The first example (lines 174-175) would work best if all your 'real participants' have a participant ID greater than a certain number, and all 'test users' have an ID lower than a certain number. This example syntax selects only participants with PIDs of 25 or higher. The second option (starting line 177) is an example of how to select only a certain set of PIDs. Select an option that will work well for your data, delete the #, edit, and execute the syntax.
...
For your information, the syntax below will print counts of diaries that were updated according to each cleaning update, as well as the count of diaries with any data changes from one or more of the cleaning updates.
...
This code creates new variable called "SleepQ" by averaging participants' sleep quality ratings over the the block of sleep diaires.
