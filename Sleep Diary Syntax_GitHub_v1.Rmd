---
title: "Sleep Diary Data - Formatting and Outcomes Computation for SHUTi for Researchers"
version: "github _v1"
author: "Kelly Shaffer, with edits and review by Lee Ritterband & Katharine E Daniel - UVA CBHT"
date: "Compiled for GitHub on 1/24/23"
output:
...
##EDITING REQUIRED: Set your Working Directory by editing the green path and executing the setwd() syntax below, or by going to Session > Set Working Directory > Choose Directory. Check your Working Directory by looking at the "Files" tab on the lower left quadrant of R Studio, or by executing the getwd() syntax below.

```{r, out.width=80}
setwd("/Volumes/CentralRoot/Research/Psychiatry and NB Sciences/PSYCH_eHealth/Data/SHUTi - other/Sleep Diary Syntax - SHUTi for Researchers")
...
##EDITING REQUIRED: There are two syntax options for you to use below. The first example (lines 145-146) would work best if all your 'real participants' have a participant ID greater than a certain number, and all 'test users' have an ID lower than a certain number. This example syntax selects only participants with PIDs of 25 or higher. The second option (starting line 148) is an example of how to select only a certain set of PIDs. Select an option that will work well for your data, delete the #, edit, and execute the syntax.
...
Next, we'll need to fix any invalid diary times. Start by computing total time in bed to help identify where there might be data entry errors. For our assessments, we compute time in bed as the total sleep opportunity, i.e., rise time - time trying to fall asleep. Compute this as tib.h (time in bed, in hours). The 'tib' variable in the downloaded dataset is computed from the original patient-entered data and will not be used once we clean the data.  

```{r}
data<- data %>% 
  mutate(tib.tot = (utctime.rise - utctime.bed)/60)           

data<- data %>% 
  mutate(tib.h = (utctime.rise - utctime.sleep)/60)
```
...
This chunk of syntax addresses that participants will sometimes confuse the date, particularly when entering midnight (i.e., 0 on 24 hour clock = 12 AM on dawning day, not 12 AM on yesterday). Date errors can only occur for bedtimes and sleep times, which can either occur the 'day before' the diary calendar date or on the same day as the diary calendar date (i.e., 10:30 PM on Monday, 1/1 or 1 AM on Tuesday, 1/2); however, wake and rise times can be reasonably assumed to occur only on the calendar date of the diary (i.e., 6 AM on Tuesday, 1/2 - in this case, the diary would be tagged with caldate = "Tuesday, 1/2").

This chunk of syntax fixes when the date has been listed incorrectly for the time to bed. We assume that users whose diary data indicate they were in bed longer than 24 hours with a bed time of 3 AM (i.e., 3 on 24-hour clock) or earlier on the day before the calendar day of the diary should have the date as the same calendar day of the diary. For example, a user indicating they went to bed at 2 AM on Monday, 1/1, who started trying to go to sleep at 2:30 AM on Tuesday, 1/2, and who reported waking and rising at 7 AM on Tuesday, 1/2.
...
This chunk of syntax fixes when the date has been listed incorrectly for the time starting to try to fall asleep. We assume that users whose diary data indicate they were in bed longer than 24 hours with a sleep time of 3 AM (i.e., 3 on 24-hour clock) or earlier on the day before the calendar day of the diary should have the date as the same calendar day of the diary. For example, a user indicating they went to bed at 2 AM on Monday, 1/1, who started trying to go to sleep at 2:30 AM on Monday, 1/1, and who reported waking and rising at 7 AM on Tuesday, 1/2.
...
We assume that users whose inputted diary data indicate they were in bed longer than 12 hours with a stated wake time of 3 PM (i.e., 15 on 24-hour clock) or later meant to input the time as AM. For example, someone inputting a bedtime of 10 PM on Monday, 1/1, who started trying to fall asleep at 10:30 PM on Monday, 1/1, who reported waking and rising at 5 PM on Tuesday, 1/2.
...
```{r}
data<- data %>% 
  mutate(tib.tot = (utctime.rise.fix - utctime.bed.fix)/60)   

data<- data %>% 
  mutate(tib.h = (utctime.rise.fix - utctime.sleep.fix)/60)
```
...
We assume that users whose inputted diary data which indicates that they were in bed longer than 12 hours with a stated bed time between 5AM and 3PM (i.e., 15 on 24-hour clock) entered AM / PM incorrectly. For example, someone inputting a bedtime of 1 PM on Monday, 1/1, who started trying to fall asleep at 2 AM on Tuesday, 1/2, with a wake and rise time of 6 AM on Tuesday 1/2.
...
We assume  that users whose inputted diary data indicate they were in bed longer than 12 hours with a stated bed time between 5 AM and 3 PM (i.e., 15 on 24-hour clock) entered AM / PM incorrectly. For example, someone inputting a bedtime of 1 PM on Monday, 1/1, who started trying to fall asleep at 2 PM on Monday, 1/1, with a wake and rise time of 6 AM on Tuesday 1/2.
...
Once we've fixed diary entry errors, there are some additional errors that can occur. These are fundamentally flawed diaries that need to be deleted due to impossible entries and not being able to make reasonable guesses at what the user intended to enter.
...
```{r}
data<- data %>% 
  mutate(sol.m = minutes(sol))   
data<- data %>% 
  mutate(waso.m = minutes(waso)) 
data<- data %>% 
  mutate(tst.fix = (utctime.wake.fix - utctime.sleep.fix)) 
data$tst.fix<- as.integer(data$tst.fix)  
data<- data %>% 
  mutate(tst.fix = minutes(tst.fix))  
data<- data %>% 
  mutate(tst.fix = tst.fix - sol.m - waso.m) 
...
data<- data %>% 
  mutate(invalid = ifelse(tib.h < minutes(0), "invalid", invalid))                       
```
...
##EDITING MAY BE REQUIRED: If there are diaries to be removed based on your findings from above, uncomment (by deleting the '#' before the line) the syntax lines below as needed. Make a record for yourself of why the diary was removed for your records (don't uncomment this line, as it is in fact a comment!):
...
##THIS IS WHERE THE ERRORS HAPPEN GO LINE BY LINE

Note for Step 1: NAs will start to be introduced for participants once you surpass the number of surveys that a given participant submitted. 
Note for Step 1: Because this code assumes four assessment time points, date1 should have no more than 4 rows per participant (but may have fewer if the participant did not complete sufficient surveys across more all four assessment blocks) 
...
intersect(check$PID, review$PID)    
```
...
```{r}
#If the output prints any PIDs, you will use the "check" database to manually 
#     review diaries for the identified participants to see whether there is 
#     a valid block of diaries across all of their submitted diaries using the 
#     decision rules numbered above.


#EXAMPLE SYNTAX FOR SELECTING ASSESSMENT DIARIES FOR USERS IN THE 'CHECK' AND 
#'REVIEW' DATABASES -- used to label specific diaryids as invalid based on your 
#'manual review of the "check" and "review" datasets:
...
```{r}
rm(addldiaries, assessment.data.addl, grouped.assessment.addl, check)   
```
...
##EDITING MAY BE REQUIRED: See the comment mid-way through this 'chunk' of syntax.
...
write.csv(assessment.data.valid, "Assessment_Sleep_Diaries.csv")  
```
...
This code creates new variable called "ema.fix" which is solved for by subtracting utctime.wake.fix from utctime.rise.fix and then dividing that value by 60 to scale in units of minutes. NOTE: because of earlier fixes, ema.fix should never be a negative value (because you can't arise before you awaken). The next part of the code creates a new variable called "wasoema", which is the sum of ema.fix + waso. Finally, the past part of the code creates an average wasoema for each participant within each assessment block and stores info into a new data frame named "wasoema". 
```{r}
...
This code first overwrites the variable called "tst.fix" by subtracting utctime.sleep.fix from utctime.wake.fix NOTE: because of earlier fixes, tst.fix should never be a negative value (because you can't go to sleep after you've awoken). The next part of the code overwrites the tst.fix that was just solved for by taking the value that was just solved for in the above code (tst.fix) and further subtracting out the waso and sol values found in the previous two chunks of code. Then, it divides that solution by 60 to scale from minutes to hours. Finally, the code creates an average tst for each participant within each assessment block and stores that info into a new data frame named "tst"
```{r}
assessment.data.valid<- assessment.data.valid %>% 
...
This code creates a new variable called "tib.fix" by subtracting utctime.sleep.fix from utctime.rise.fix. Then scales from minutes to hours by dividing by 60. NOTE: because of earlier fixes, tib.fix should never be negative value (because you can't go to arise without first going to sleep). Finally, the code creates an average tib for each participant within each assessment block and stores info into a new data frame named "tib". 
```{r}
assessment.data.valid<- assessment.data.valid %>% 
...
In this section we will build a data frame, called "indat", which will contain information about how a user interacted with the program during the 9-week intervention period (i.e., between Day 0 and Day 62). The variables that we will calculate in this section are: Int_Diary_Count; Int_Login_Count; PE_initiated; and PE_completed. Descriptions for each of these variables will be provided before each relevant code chunk. After building the "intdat" data frame, we will merge this information into all other outcomes to arrive at our final assessment outcomes data set.

First, we will calculate "Int_Diary_Count", which is a count variable that reflects the number of diaries a participant entered during the 9-week intervention period. Only participants assiged to the SHUTi experimental condition will have >0 intervention diaries, as diary-logging is exclusively a feature of SHUTi and not available as part of PE. These diaries are separate of those completed as part of assessment periods.
```{r}
...
Next, we will calculate "Int_Login_Count", which is a count variable that reflects the number of "meaningful logins" that a participant made to their assigned online intervention program during the 9-week intervention period. We define a "meaningful login" as all unique logins that did not take place within 5 minutes of another login. The below code removes logins that occurred within 5 minutes of a previously counted login to avoid inflating a user's "Int_Login_Count" due to frequent clicking into and out of the intervention program without the user having time sufficient time to engage with the program. Logins are counted both for those assigned to the experimental SHUTi condition, as well as to the online Patient Education control condition.
```{r}
...
Next, we will calculate "PE_completed", which is a binary variable that reflects whether or not a user ever completed Patient Education (PE) control condition program. This variable is scored 1 if the user, at any point during the 9-week intervention period, completed PE (i.e., clicked through the program and clicked "complete" button at conclusion of program). It is scored 0 if the user never completed PE during the intervention period.
```{r}
...
Here, we create a new long (one row per participant per timepoint [e.g., 100 participants x 4 time points = 400 rows]) & wide (one row per participant [e.g., 100 participants x 4 time points = 100 rows with additional columns]) dataset with participants' number of diaries used in the block and the five sleep diary metrics listed. 

Prior to saving the final data as a csv file, we will add in the information contained in the "intdat" data frame. 
...
final.assessment<- merge(n.block, sol, 
                         by = c("PID", "assessment"))
final.assessment<- plyr::join_all(list(final.assessment, wasoema, tst, tib, se), 
                             by = c("PID", "assessment"))
final.assessment.long<- final.assessment                            
final.assessment.long <- left_join(final.assessment.long, intdat, by = "PID") 

#write.csv(final.assessment.long, "Final_Assessment_Diary_Outcomes_Long.csv")  
...
colnames(final.assessment.wide)<-c("PID", "assessment.1", "n.block.1", "sol.1", 
                                   ...
                                   "assessment.2", "n.block.2", "sol.2", "wasoema.2", 
                                   "tst.2", "tib.2", "se.2", "assessment.3", 
                                   "n.block.3", "sol.3", "wasoema.3", "tst.3", 
                                   "tib.3", "se.3", "assessment.4", "n.block.4", 
                                   "sol.4", "wasoema.4", "tst.4", "tib.4", "se.4")       
                                                                                       

final.assessment.wide <- left_join(final.assessment.wide, intdat, by = "PID") 

#write.csv(final.assessment.wide, "Final_Assessment_Diary_Outcomes_Wide.csv")
